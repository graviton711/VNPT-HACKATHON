# SUBMISSION GUIDELINE - Track 2 The Builder v1.0
...
**VD:** `nvidia/cuda:12.2.0-devel-ubuntu20.04`
...
3.  Kiểm tra xem file `submission.csv`, `submission_time.csv` có được sinh ra sau khi container chạy xong hay không.
...
3.  Xuất kết quả ra file `submission.csv`, `submission_time.csv` (xem hướng dẫn bên dưới) theo định dạng quy định của BTC.
...
Đối với phần chấm điểm thời gian inference, đề nghị các đội chạy theo vòng `for` theo từng item trong pipeline để có thể lấy được thời gian chạy của từng sample, từ đó BTC có căn cứ chấm điểm thời gian inference của các đội một cách công bằng nhất.

Các đội có thể tham khảo code dưới đây để lấy thời gian chạy của từng câu trong tập test:

```python
for item in test: # test là list các câu test
    start = time.time()
    # --- chạy pipeline của đội tại đây ---
    # ví dụ:
    # output = model.inference(item)
    # -------------------------------------
    end = time.time()
    time_infer_sample = end - start
```

Sau đó, các đội tiến hành ghi thêm file `submission_time.csv` có chứa thông tin của cột `time` cho từng sample.

| File | Cột | Ví dụ |
| :--- | :--- | :--- |
| `submission.csv` | `qid`, `answer` | `test_0001,A` |
| `submission_time.csv` | `qid`, `answer`, `time` | `test_0001,A,1.2345` |
| | | `test_0002,B,2.9087` |
| | | `test_0003,C,1.0021` |
| | | `test_0004,D,3.4509` |
