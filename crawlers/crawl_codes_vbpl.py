...
# Logic: Crawl "Bộ luật" (Codes) from VBPL.vn
# Scope: All found on first 5 pages (Codes are rare and important).
# URL: idLoaiVanBan=16

BASE_URL = "https://vbpl.vn/TW/Pages/vanban.aspx?idLoaiVanBan=16&dvid=13&Page={}"
DOMAIN = "https://vbpl.vn"
OUTPUT_FILE = r"e:\VSCODE_WORKSPACE\VNPT\data\codes_vbpl.json"
...
def crawl():
    results = []
    # Codes are few, let's crawl 5 pages to be safe, probably fewer than 50 total.
    PAGES_TO_CRAWL = 5 
    
    print(f"Starting crawl for 'Bộ luật' (Codes)...")
            ...
            for li in items:
                ...
                if div_right:
                    for p in div_right.find_all("p"):
                        ...
                        if "Trạng thái:" in text and "Hết hiệu lực toàn bộ" in text:
                            should_skip = True
                            print(f"  [SKIP] {title} - Hết hiệu lực toàn bộ")
                            break
                
                if should_skip:
                    continue

                print(f"  [FOUND] {title} ({issuance_date_str})")
    ...
    print(f"Crawl finished. Collected {len(results)} codes.")
